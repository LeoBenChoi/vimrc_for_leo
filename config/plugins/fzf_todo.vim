"==============================================================
" config/plugins/fzf_todo.vim
" TODO 搜索功能：使用 FZF 搜索代码中的 TODO 注释
"==============================================================

if exists('g:loaded_fzf_todo_config')
    finish
endif
let g:loaded_fzf_todo_config = 1
...
if !exists('g:loaded_fzf_config')
    echohl WarningMsg
    echomsg '[fzf_todo.vim] 警告: fzf.vim 未加载，TODO 搜索功能需要 fzf 支持'
    echohl None
    finish
endif
...
function! s:todo_search() abort
    ...
    if executable('rg')
        ...
        " --sort path: 按路径排序
        let l:grep_cmd = 'rg --column --color=always --no-messages --no-heading --sort path -i "TODO"'
    elseif executable('ag')
        " 使用 ag 搜索
        let l:grep_cmd = 'ag --column --color --no-messages --noheading -i "TODO"'
    else
        echohl ErrorMsg
        echomsg '[fzf_todo.vim] 错误: 未找到搜索工具 (rg/ag)，请安装 ripgrep 或 ag'
        echohl None
        return
    endif
    ...
    if exists('*fzf#vim#grep')
        ...
        echohl ErrorMsg
        echomsg '[fzf_todo.vim] 错误: fzf.vim 未加载，无法使用 TODO 搜索功能'
        echohl None
    endif
endfunction
...
function! s:todo_file_search() abort
    ...
    if empty(expand('%'))
        echohl ErrorMsg
        echomsg '[fzf_todo.vim] 错误: 当前没有打开的文件'
        echohl None
        return
    endif
    ...
    if executable('rg')
        ...
        let l:grep_cmd = 'rg --column --color=always --no-messages --no-heading -i "TODO" ' . l:file_path_escaped
    elseif executable('ag')
        " 直接复用 todo_search 的逻辑，只需在命令后添加文件路径
        let l:grep_cmd = 'ag --column --color --no-messages --noheading -i "TODO" ' . l:file_path_escaped
    else
        echohl ErrorMsg
        echomsg '[fzf_todo.vim] 错误: 未找到搜索工具 (rg/ag)，请安装 ripgrep 或 ag'
        echohl None
